...
class Converter():
    ...
    def __init__(self, output_size, coverage_ratio, draw_transparent, pre_encode,
        ...
        self._scale = arguments.output_scale / 100 if hasattr(arguments, "output_scale") else 0.64
    ...
    def _load_plugins(self, config=None, disable_logging=False):
        ...
        self._adjustments["mask"] = PluginLoader.get_converter(
            "mask",
            "mask_blend",
            disable_logging=disable_logging)(self._args.mask_type if hasattr(self._args, "mask_type") else None,
                                             self._output_size,
                                             self._coverage_ratio,
                                             configfile=self._configfile,
                                             config=config)

        if hasattr(self._args, "color_adjustment"):
