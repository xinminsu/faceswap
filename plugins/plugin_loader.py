...
class PluginLoader():
    ...
    @staticmethod
    def get_live(category, name, disable_logging=False):
        """ Return requested live plugin

        Converters work slightly differently to other faceswap plugins. They are created to do a
        specific task (e.g. color adjustment, mask blending etc.), so multiple plugins will be
        loaded in the live phase, rather than just one plugin for the other phases.

        Parameters
        ----------
        name: str
            The name of the requested live plugin
        disable_logging: bool, optional
            Whether to disable the INFO log message that the plugin is being imported.
            Default: `False`
        ...
        Returns
        -------
        :class:`plugins.live` object:
            A live sub plugin
        """
        return PluginLoader._import("live.{}".format(category), name, disable_logging)
